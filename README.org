...
You can also cache to disk all the Roam buffer contents (bearing in mind that this stores potentially world-readable text under "~/.emacs.d"), so they do not need re-generating if they have not changed:

#+begin_src elisp
(org-node-fakeroam-setup-persistence)
#+end_src
