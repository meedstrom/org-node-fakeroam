...
Optionally, you can enhance it by persisting on disk all the preview snippets.  The only reason it's not done by default is that it stores potentially world-readable text under "~/.emacs.d", and it's up to you to judge if that's a problem.
...
PROVISO: if you use the [[https://github.com/jkitchin/org-ref][org-ref]] package, do not proceed!
